...
export class Movie {
    ...
    backdropURLForWidth(imageWidth: number, imageHeigth: number): string {
        ...
        const imageSize = this.findImageSizeFor(imageWidth, imageHeigth, this.available_backdrop_sizes);
    ...
    posterURLForWidth(imageWidth: number, imageHeigth: number): string {
        ...
        const imageSize = this.findImageSizeFor(imageWidth, imageHeigth, this.available_poster_sizes);
        return `${this.images_base_url}${imageSize}${this.poster_path}`;
    }

    private findImageSizeFor(imageWidth: number, imageHeigth: number, available_sizes: image_size[]) {
        const GR = 1.62;
        ...
        return available_sizes.find(available_size => Number(available_size.slice(1)) > requiredImageWidth) || available_sizes.pop();
    }
